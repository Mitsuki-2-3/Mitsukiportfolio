...
typedef struct tagVECTOR
{
	...
	tagVECTOR operator+=(tagVECTOR _data)
	{
		this->x += _data.x;
		this->y += _data.y;
		this->z += _data.z;
	...
	{
		VECTOR localdata;
		localdata.x = this->x + _data.x;
		localdata.y = this->y + _data.y;
		localdata.z = this->z + _data.z;
	...
	tagVECTOR operator*(int _data)
	{
		VECTOR localdata;
		localdata.x = this->x * _data;
		localdata.y = this->y * _data;
		localdata.z = this->z * _data;
	...
	tagVECTOR operator/(int _data)
	{
		...
		return localdata;
	}tagVECTOR operator*(float _data)
	{
		VECTOR localdata;
		localdata.x = this->x * _data;
		localdata.y = this->y * _data;
	...
	//{
	//	VECTOR localdata;
	//	localdata.x = floor(this->x);
	//	localdata.y = floor(this->y);
	//	localdata.z = floor(this->z);
	...
	tagVECTOR VFloor()
	{
		...
		this->x = floorf(this->x * p) / p;
		this->y = floorf(this->y * p) / p;
		this->z = floorf(this->z * p) / p;

		return *this;
	}
	bool operator==(tagVECTOR _data)
	{
		if (this->x == _data.x && this->y == _data.y && this->z == _data.z)
		{
			return true;
		}
	...
	float innerproduct(tagVECTOR _data)
	{
		//二つの座標から内積を計算
		float localdata = 0;

		localdata = (this->x * this->x + _data.x) + (this->y * this->y + _data.y) + (this->z * this->z + _data.z);

		return localdata;
	}
	float distance(tagVECTOR _data)
	{
		float lol = 0;

		lol = powf((this->x - _data.x), 2) + powf((this->y - _data.y), 2) + powf((this->z - _data.z), 2);
...
// カウンタ及び時刻取得系関数
extern	int			GetNowCount(							int UseRDTSCFlag = FALSE ) ;	// ミリ秒単位の精度を持つカウンタの現在の値を得る
extern	LONGLONG	GetNowHiPerformanceCount(				int UseRDTSCFlag = FALSE ) ;	// GetNowCountの高精度バージョン( μ秒単位の精度を持つカウンタの現在の値を得る )
...
extern	int			GetMouseInputLog2(		int *Button, int *ClickX, int *ClickY, int *LogType, int LogDelete = TRUE ) ;	// マウスのボタンを押したり離したりした情報を一つ取得する( Button:押されたり離されたりしたボタン( MOUSE_INPUT_LEFT 等 )を格納する変数のアドレス  ClickX:押されたり離されたりした時のＸ座標を格納する変数のアドレス  ClickY:押されたり離されたりした時のＹ座標を格納する変数のアドレス  LogType：押されたのか( MOUSE_INPUT_LOG_DOWN )離されたのか( MOUSE_INPUT_LOG_UP )、等の情報を格納する変数のアドレス  LogDelete:取得した押されたり離されたりした情報一つ分をログから削除するかどうか( TRUE:削除する  FALSE:削除しない、つまり次にこの関数が呼ばれたときに同じ値を取得することになる )　　戻り値  0:押されたり離されたりした情報取得できた　-1:押されたり離されたりした情報が無かった、つまり前回の呼び出し( または起動時から最初の呼び出し )の間に一度もマウスのボタンが押されたり離されたりしなかった )
#endif // DX_NON_INPUT
...
extern	int			AddStreamSoundMemToMem(              const void *FileImage, size_t FileImageSize, int LoopNum,  int SoundHandle, int StreamDataType, int UnionHandle = -1 ) ;		// ストリーム再生タイプのサウンドハンドルにメモリ上に展開したサウンドファイルイメージを再生対象に追加する
...
extern	int			LoadSoundMemByMemImageBase(          const void *FileImage, size_t FileImageSize, int BufferNum,      int UnionHandle = -1 ) ;				// メモリ上に展開されたサウンドファイルイメージからサウンドハンドルを作成する
extern	int			LoadSoundMemByMemImage(              const void *FileImage, size_t FileImageSize, int BufferNum = 3 , int UnionHandle = -1 ) ;				// LoadSoundMemByMemImageBase の別名関数
...
extern	int			MakeSoftSoundCustom(				int ChannelNum, int BitsPerSample, int SamplesPerSec, LONGLONG SampleNum, int IsFloatType = 0 ) ;	// ソフトウエアで扱う空の波形データハンドルを作成する
